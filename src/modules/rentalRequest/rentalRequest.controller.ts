...
const getLandlordRentalRequests = catchAsync(async (req: Request, res: Response) => {
    const result = await RentalRequestServices.getLandlordRentalRequestsFromDB(req.user?.id as string);
...
const updateRentalRequestStatus = catchAsync(async (req: Request, res: Response) => {
    ...
    const requestId = req.params.id as string
    const result = await RentalRequestServices.updateRentalRequestStatusInDB(
        requestId,
        landlordId,
        req.body
    );
